...
def  generate_CrossLine_pattern(line_width):
    ...
    img[:,width//4-line_width:width//4+line_width]=255
    img[:,width*3//4-line_width:width*3//4+line_width]=255
    return img
...
def AddRoiPattern(img_in,size_ratio,shift_ration):
    ...
    for i in range(0,4):
        ...
        x=(wid*3//4)-shift_ration*wid//4+i%2*shift_ration*wid*2//4
...
def gamma_white(ratio):
    img=np.zeros((height,width),dtype=np.uint8)
    img[int(height//2-height*ratio//2):int(height//2+height*ratio//2),int(width*3//4-height*ratio//2):int(width*3//4+height*ratio//2)]=255
    return img
    
def gamma_haha(ratio):
    img=np.zeros((height,width),dtype=np.uint8)
    for width_pos in range(int(width*3//4-height*ratio//2),int(width*3//4+height*ratio//2)):
        value=(width_pos-int(width*3//4-height*ratio//2))*255//(int(width*3//4+height*ratio//2)-int(width*3//4-height*ratio//2))
...
if __name__ == '__main__':
    ...
    ROI=AddRoiPattern(cross,0.5,0.4)
