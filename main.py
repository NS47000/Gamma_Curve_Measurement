...
def generate_ROI_pattern():
    ...
    ROI=pattern_gamma.AddRoiPattern(cross,0.5,0.4)
    return ROI
...
def find_squares(img):
    ...
    return squares, img
def test_exp(ROI):
    gray_pattern=pattern_gamma.AddGrayPattern(ROI,0.5,0.4,63,127,191,255)
    ...
    roi_list4=[]
    exp_list=[]
    for exp in range(11111,11111,11111):
...
if __name__ == '__main__':
    ...
    data_detect=[]
    data_show=[]
    white_pattern=pattern_gamma.AddGrayPattern(ROI,0.5,0.4,255,255,255,255)
    ...
    black_pattern=pattern_gamma.AddGrayPattern(ROI,0.5,0.4,0,0,0,0)
    ...
    for grayLevel in range(0,252,4):
        ratio=0
        gray_pattern=pattern_gamma.AddGrayPattern(ROI,0.5,0.4,grayLevel,grayLevel+1,grayLevel+2,grayLevel+3)
        Q35_showpattern(gray_pattern)
        if grayLevel<64:
            ratio=15
        elif grayLevel<128:
        ...
        data_detect.append((np.mean(np.mean(roi_1))+7-roi_1_black*ratio)/ratio*255/roi_1_balance)
        data_detect.append((np.mean(np.mean(roi_2))+7-roi_2_black*ratio)/ratio*255/roi_2_balance)
        data_detect.append((np.mean(np.mean(roi_3))+7-roi_3_black*ratio)/ratio*255/roi_3_balance)
        data_detect.append((np.mean(np.mean(roi_4))+7-roi_4_black*ratio)/ratio*255/roi_4_balance)
    fig=plt.figure()
    plt.title("Gamma curve ")
    plt.plot(data_show, data_detect, 'go-', linewidth=3)
