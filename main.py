...
def find_squares(img):
    ...
    for cnt in contours:
        ...
        if  cv2.contourArea(cnt)>img.shape[0]*img.shape[1]//100:#and cv2.isContourConvex(cnt) and len(cnt) == 4 
            print("Area:"+str(cv2.contourArea(cnt))+"cnt_len:"+str(len(cnt)))
...
def test_exp(ROI):
    ...
    tiltfive_showpattern(gray_pattern)
    roi_list1=[]
    roi_list2=[]
    roi_list3=[]
    roi_list4=[]
    exp_list=[]
    for exp in range(160000,360000,20000):
...
if __name__ == '__main__':
    ...
    if Needto_capture==True:
        ...
        tiltfive_showpattern(white_pattern)
    ...
    if Needto_capture==True:
        ...
        tiltfive_showpattern(black_pattern)
    ...
    for i in range(0,len(data_detect),4):
        ...
        data_detect_2[i//4]=data_detect[i]
    ...
    plt.grid(True)
    plt.show() 
    fig.savefig("T5_Gamma_curve.png")
    path = 'T5_result.csv' 
    with open(path,'w') as f:
